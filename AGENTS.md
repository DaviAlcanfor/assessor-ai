...
e tools estão no [README.md](README.md) — leia-o antes de mexer em `graph/agents/` ou `graph/tools/`.

## Stack

- Python 3.13+, gerenciado com `uv` (`uv venv`, `uv sync`, `uv add <pkg>`)
- LangChain 1.2 / LangGraph 1.1 para orquestração de agentes
- LLMs: Gemini (`gemini-2.5-flash`) e Groq (`openai/gpt-oss-120b`), com Claude e Qwen mapeados em
  `models.py` mas ainda não usados por nenhum agente
- PostgreSQL para transações, eventos e checkpoint do LangGraph, acessado via SQLAlchemy ORM (`graph/tools/{financeiro,agenda,usuarios}/models.py`) +
  Alembic pra migrations
- MongoDB para histórico de conversa e perfil de usuário
- Qdrant para RAG do FAQ (`graph/tools/faq/`) — substituiu o FAISS local
- Redis para rate limit por usuário, alocação de API key e cache de perfil (`infra/redis.py`)
- Postgres/Mongo/Redis/Qdrant são todos serviços em nuvem — não há infra local via Docker; `.env`
  aponta direto para os serviços hospedados
- FastAPI (`api/`) com auth por API key, rate limiting (`slowapi` por IP + Redis por
  ...
  Textual (`tui/`) pra TUI
- `pytest` (`tests/`, espelhando a estrutura de `src/assessor_ai/`) + `ruff` (lint/format) + CI no GitHub
  Actions
...
repositories/ chat_repository.py — fachada sobre graph/tools/chats, graph/tools/usuarios e infra/cache
...
graph/      agentes compilados, nodes, prompts, tools, state.py, llm.py e builder.py
infra/      conexões compartilhadas (postgres, mongo, redis, qdrant) e cache de perfil
config.py, models.py, logging.py, privacy.py, identifiers.py
...
de `graph/tools/` diretamente — sempre via `services/chat_service.py`, que por sua vez fala com
...
interno (chats, usuarios) é só chamado por `repositories/`. Conexão nova vai em `infra/`, nunca
...
5. **Escopo** — `services/runner.py:executar` seta o `ContextVar` de `user_id` e chama
   `fluxo_agentes().ainvoke(...)`. Toda tool lê o usuário daí, nunca dos args escolhidos pelo LLM.
6. **Grafo** — guardrail de entrada → router → especialista (tools) → orquestrador → guardrail de
   saída (fluxo detalhado no README).
7. **Persistência** — a resposta volta pro `service`, que grava o histórico no Mongo e o checkpoint
   do LangGraph no PostgreSQL; `encerrar_sessao` gera resumo/perfil e invalida o cache de perfil no Redis.
...
- Enums de domínio usam `StrEnum` quando precisam de validação em runtime (ver `graph/state.py:Route`).
  Para conjuntos fechados usados apenas na tipagem estática, prefira `Literal` e constantes (ver
  `graph/agents/nodes/names.py:NodeName`).
...
- **Tools do LLM nunca recebem `user_id` como argumento.** Args de tool são escolhidos pelo LLM via
  ...
  (`infra/postgres.py:current_user_id()`, exposto como `self.usuario` no `PostgresRepo`). Ver uso em `graph/tools/{financeiro,agenda}/repo.py`.
...
- **Package by feature nas tools.** `graph/tools/{financeiro,agenda,faq,chats,usuarios}` — uma pasta por
  ...
  mesmo corte: `graph/tools/<sistema>/<domínio>/`.
- **Repository leve por domínio.** `core.py` expõe as operações (`buscar`, `criar`, `atualizar_*`)
  ...
  soltas + funções.
- **Infra isolada e lazy.** Toda conexão externa (`infra/{postgres,mongo,redis,qdrant}.py`)
...
- **ORM sobre Postgres, mas cada tool continua com seu próprio `try/except`.**
  `graph/tools/{financeiro,agenda,usuarios}/models.py` tem os models declarativos; `infra/postgres.py:get_session()` já faz
  ...
  (`logging.py`) não captura exceção nenhuma, só inspeciona `result["status"]`, então uma
...
- **Single responsibility por nó de agente.** `graph/agents/nodes/` (execução) fica separado de
  ...
  opcional (`usa_tools_obrigatorias: true`). `graph/agents/prompts/loader.py` é o único `.py` da pasta —
...
- **Contrato de retorno único.** Tools não retornam dict cru nem deixam exception vazar para o
  agente — usam `Response` (`graph/tools/response.py`) como envelope padrão de sucesso/erro. Ao criar
  tool nova, reusar essa classe em vez de inventar outro formato de retorno.
- **Config centralizada.** Uma única fonte de env vars (`config.py`, `pydantic-settings`, com
  toda credencial — inclusive URLs de conexão — como `SecretStr`) e um único enum fechado de
  modelos/providers (`models.py:Model`/`PROVIDER_MAP`). Não ler
  `os.environ` direto em outros módulos.
- **Entrypoint fino.** `main.py` só faz dispatch por argv (`terminal`/`tui`/`api`) — nenhuma lógica
  de negócio nele. Lógica de negócio nova vai em `services/`, nunca de volta pra `main.py`.
...
1. Criar `graph/tools/<feature>/schemas.py` com os modelos Pydantic de entrada/saída (e `models.py`, se
   usar Postgres — nesse caso importar o model no `alembic/env.py`).
2. Criar `graph/tools/<feature>/repo.py` com uma classe `*Repo`. Se for Postgres, herdar de `PostgresRepo`
...
4. Se for um banco/serviço novo, criar a conexão em `infra/` com init lazy — nunca dentro da
   pasta da feature.
5. Se a tool precisa ser escopada por usuário, usar `self.usuario` — nunca adicionar `user_id` ao
   `args_schema` da tool.
6. Instanciar o repo em `graph/tools/__init__.py` e registrar a lista no agente em `graph/agents/__init__.py`.
