...
from assessor_ai.tools.faq.schemas import FaqRetrieverArgs, SearchResponse
from assessor_ai.tools.infra.qdrant import QdrantConn, qdrant
    ...
    def faq_retriever(self, question: str) -> dict:
        ...
        try:
            ...
            if not self.conn.client.collection_exists(colecao):
                logger.error(
                    "FAQ ERRO | collection '%s' não existe — rode: python -m assessor_ai.tools.faq.ingest",
                    colecao,
                )
